...
import { PolicyDetailsView } from '@/components/policies/PolicyDetailsView';
import type { Profile } from '@/types/auth';
import type { Policy, PolicyDocument, PolicySection } from '@/types/policy';

export const metadata: Metadata = {
  title: 'Policy Details & Original Wording — PRISM Insurance Companion',
  description:
    'Deep dive into your policy clauses, original verbatim wording, coverage rules, and in-document search.',
};
...
export default async function PolicyDetailsPage({
  ...
  if (!user) {
    redirect(`/login?next=/policies/${id}/details`);
  }
    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#F6F8FB] dark:bg-[#0B1220]">
        ...
        <main className="flex-1 max-w-md w-full mx-auto px-4 py-16 flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-sm text-[#667085] dark:text-slate-400 leading-relaxed">
            PRISM is currently extracting text and indexing sections from your policy document. Detailed clauses will appear once indexing is complete.
          </p>
